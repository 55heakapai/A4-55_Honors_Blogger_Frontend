...
export const fetchAuthor = createAsyncThunk(
    ...
    async (arg, { rejectWithValue }) => {
        try {
            const response = await axios.get('http://localhost:3001/author/authlist');
...
export const AddAuthorData = createAsyncThunk(
    ...
    async (addAuthorInputData, { rejectWithValue }) => {
        ...
        try {
            const response = await axios.post('http://localhost:3001/author/addAuth', addAuthorInputData, {
                headers: {
...
export const DeleteAuthor = createAsyncThunk(
    ...
    async (autID, { rejectWithValue }) => {
        try {
            const response = await axios.delete(`http://localhost:3001/author/deleteAuth/${autID}`, {
                headers: {
                    token: localStorage.getItem('token'),
                },
            });
...
export const UpdateAuthor = createAsyncThunk(
    ...
    async ({ID,Data}, { rejectWithValue }) => {
        try {
            const response = await axios.put(`http://localhost:3001/author/updateAuth/${ID}`, Data, {
                headers: {
                    token: localStorage.getItem('token'),
                },
            });
